package com.trendyol.android.devtools.autofillservice.internal
...
import com.trendyol.android.devtools.autofillservice.R
...
internal class AutofillProcessor(
    ...
    init {
        ...
        launchIO {
            ...
            autofillData = runCatching {
                ...
                .onFailure {
                    Log.e("Autofill", "Exception occurred while reading $filePath: ${it.message}", it)
                    application.unregisterActivityLifecycleCallbacks(this@AutofillProcessor)
                }
                .onSuccess {
                    Log.d("Autofill", "$filePath is read.\n$it")
                }
                .getOrNull()
        }
    }
    ...
    private fun processView(
        ...
        val inputs = view?.findAllInputs()
            .orEmpty()
            .associateBy { application.resources.getResourceEntryName(it.id) }
                ...
                when (item) {
                    ...
                    is ListItem.Autofill -> {
                        matchedForm.fields.forEachIndexed { index, inputId ->
                            val editText = inputs[inputId] ?: return@forEachIndexed
                            if (editText.isShown) {
                                editText.setText(item.data[index])
                            }
                        }
                        true
                    }
    ...
    private fun findMatchedForm(inputs: Map<String, EditText>): Form? {
        return autofillData?.data.orEmpty().find { form ->
            if (form.matchAnyField == true) {
                inputs.keys.any { key -> form.fields.any { it == key } }
            } else {
                inputs.keys.containsAll(form.fields)
            }
        }
    }
